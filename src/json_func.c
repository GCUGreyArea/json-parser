...
/**
 * @brief Render the list of tokens into a hash map of paths
 *
 */

inline static bool render(struct json_token *list, hashMap_t *hash_map, unsigned int *tkn, char *string)
{
    // We can only parse a list or an object
    if (list[*tkn].type != JSON_LIST && list[*tkn].type != JSON_OBJECT)
    {
        printf("invalid token list start object %s\n", get_token_type_string(list[*tkn].type));
        return false;
    }

    struct json_token *upper = list[*tkn].upper;

    // Get the object size to parse
    unsigned int size = list[*tkn].size;
    for (unsigned int i = *tkn + 1; i < size; i++)
    {
        switch (list[i].type)
        {
        case JSON_FLOAT:
...
{
    hashMapInit_t init = {"token paths", NULL, NULL, NULL, NULL, 1024};
    hashMap_t *map = hashMap_make(&init);
    struct json_token *list = get_token_list(tokens);
    unsigned int size = get_token_list_size(tokens);

    unsigned int tkn = 0;
    render(list, map, &tkn, string);
